...
## Release Checklist (CI-managed artifacts)
- **Version bumps (required before tagging)**:
  - `Cargo.toml` (`version = "X.Y.Z"` for Rust CLI)
  - `pyproject.toml` (`version = "X.Y.Z"` for PyPI shim `pybun-cli`)
  - `CHANGELOG.md` (add a section for `vX.Y.Z`)
- **CI-owned outputs (do not hand-edit)**:
  - `release-metadata/` (generated by `release.yml` after assets are built/signed)
  - `Formula/pybun.rb`, `winget/pybun.yaml`, `bucket/pybun.json` (updated via `scripts/release/generate_package_managers.py` in CI once `release-metadata` is available)
