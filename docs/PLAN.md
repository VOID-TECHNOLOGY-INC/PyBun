...
- PR-A3: MCP と CLI の実処理統一
  ...
  - [DONE] Issue #272: `mcp.rs::call_run` が Python interpreter discovery・sandbox 設定・実行/タイムアウト処理を CLI `pybun run`（`commands::run_script` / `run_python_code`）から独立して再実装しており、将来の CLI 側の挙動変更が MCP 側へ自動的に反映されない divergence risk があった。
    ...
    - Note: PR-A3 本体（MCP/CLI の実処理統一）としては `pybun_resolve` など他ツールの統合が引き続き未完了。
