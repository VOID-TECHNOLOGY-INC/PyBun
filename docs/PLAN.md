...
- PR-A3: MCP と CLI の実処理統一
  ...
  - [DONE (partial: pybun_run execution primitive only)] Issue #272: `mcp.rs::call_run` が Python interpreter discovery・sandbox 設定・実行/タイムアウト処理を CLI `pybun run`（`commands::run_script` / `run_python_code`）から独立して再実装しており、将来の CLI 側の挙動変更が MCP 側へ自動的に反映されない divergence risk があった。
    ...
    - Tests: `src/sandbox.rs` の `tests` モジュールに4件追加 — `execute_with_optional_sandbox_plain_no_capture_uses_status`、`execute_with_optional_sandbox_plain_capture_returns_output`、`execute_with_optional_sandbox_honors_sandbox_timeout`（1秒タイムアウトで `sleep 5` を実行し `timed_out` を確認）、`execute_with_optional_sandbox_sandboxed_completion_captures_output`。これらは CLI/MCP 双方が委譲する共有実行プリミティブを直接検証するため、将来どちらかの呼び出し側だけ変更されて挙動が分岐した場合に検知できる。既存テスト（`cargo test`: lib 443件 + 統合テスト全スイート、`cargo test mcp`: 76件）はすべてパス。`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` パス。
