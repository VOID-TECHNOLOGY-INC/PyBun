...
- PR-A3: MCP と CLI の実処理統一
  ...
  - [IN PROGRESS (pybun_run execution primitive shared)] Issue #272: `mcp.rs::call_run` が Python interpreter discovery・sandbox 設定・実行/タイムアウト処理を CLI `pybun run`（`commands::run_script` / `run_python_code`）から独立して再実装しており、将来の CLI 側の挙動変更が MCP 側へ自動的に反映されない divergence risk があった。
    - Current: interpreter discovery（`find_python_env()`）は元々 MCP・CLI 双方が同一関数を呼んでおり分岐していなかったことを確認。実際に分岐していたのは「サンドボックス適用後のコマンド実行（sandboxed/plain の分岐・タイムアウト処理・stdout/stderr capture）」で、これが `run_script`・`run_python_code`・`mcp::call_run` の3箇所にほぼ同一のロジックとして重複していた。`src/sandbox.rs` に共有プリミティブ `execute_with_optional_sandbox(cmd, sandbox_guard: Option<&SandboxGuard>, capture_output: bool) -> Result<SandboxedExecution>` を新設し、3箇所すべてをこの関数へ委譲するよう置き換え。`SandboxConfig` の構築（CLI フラグ vs MCP `sandbox_policy` JSON）と応答フォーマットは各呼び出し側の責務のまま維持（入力形状が異なるため統一不可能かつ非目標）。CLI `pybun run` の外部から観測可能な挙動（sandboxed/plain 実行結果・タイムアウト・出力）は変更なし。MCP `pybun_run` の応答 JSON 形状（`sandboxed`/`audit`/`resource_limits`/`dry_run` 等）も変更なし。
    - Tests: `src/sandbox.rs` の `tests` モジュールに5件追加 — plain status/output capture、Linux sandbox timeout、sandbox output capture、raw `std::io::Error` preservation をUnix/Windows別のcommand fixtureで検証。これらは CLI/MCP 双方が委譲する共有実行プリミティブを直接検証し、caller固有のerror contextを維持する。既存テスト（`cargo test`: 997件成功・6件ignore、`cargo test mcp`: 76件）はすべてパス。`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` パス。
    - Note: `pybun_run` の PEP 723 自動インストールなど、CLI `run_script` の他のロジック（キャッシュ・プロファイル等）への統合は引き続き A3 本体のスコープとして未完了。
