...
- [DONE] PR-A9: stale pre-0.1.19 PyPI cache causes fatal `E_RESOLVE_IO`; gc/doctor cache path coverage (Issue #202)
  - Goal: v0.1.19 で `CacheEntry`（`src/pypi.rs`）に `body: Vec<u8>` フィールドが追加されたため、それ以前の pybun が書き込んだ `.bin` キャッシュエントリを `bincode::deserialize` すると decode エラーになり、`pybun add`/`install`/`lock` がフェイタルな `E_RESOLVE_IO` で失敗する regression があった。また `pybun doctor`/`pybun gc` は `~/.cache/pybun`（`Cache::new()`）のみを報告し、実際の PyPI メタデータキャッシュ（`dirs::cache_dir()/pybun/pypi`、macOS では `~/Library/Caches/pybun/pypi`）は gc/doctor の対象外だった。
  - Current: **Part A** — `src/pypi.rs::load_cache_from_paths` を `(Option<CacheEntry>, Option<String>)` を返すよう変更し、既存 `.bin` の bincode decode エラーは fatal ではなく cache miss（`Ok((None, Some(notice)))`）として扱う。`PyPiClient` に `stale_cache_notices: Arc<Mutex<Vec<String>>>` を追加し、`take_stale_cache_notices()`（`PyPiClient`/`PyPiIndex` 両方に実装）で破棄された stale エントリの notice を取り出せるようにした。`src/commands/mod.rs` の `install`/`lock_dependencies`/PEP723 native run/`run_outdated`/upgrade の各 resolve 呼び出し後に notice を drain し `collector.warning(...)` として diagnostics に出力。**Part B** — `src/pypi.rs` に `pypi_cache_dir()`（`PYBUN_PYPI_CACHE_DIR` または `dirs::cache_dir()/pybun/pypi`）、`pypi_cache_stats()`（entry_count/total_bytes/stale_count）、`gc_stale_pypi_cache(dir, dry_run)`（stale `.bin` の削除または dry-run 報告）を追加。`run_doctor` に `pypi_cache` チェックを追加（stale エントリがあれば info 診断と `pybun gc` 実行ヒントを出力）。`run_gc` の `freed_bytes`/`files_removed`/`would_remove`/サマリーに pypi cache の GC 結果を合算し、JSON detail に `pypi_cache` セクション（`path`/`freed_bytes`/`files_removed`/`would_remove`）を追加。
  - Tests: `src/pypi.rs` の `tests` モジュールに `stale_bincode_cache_is_treated_as_cache_miss`（pre-`body`-field レイアウトの `.bin` を書き込み、`load_cache` が `Ok(None)` を返し notice が drain できることを検証）を追加（5 tests in `pypi::` 全パス）。`tests/gc.rs` に統合テスト3件を追加 — `gc_dry_run_reports_stale_pypi_cache_entries`（`PYBUN_PYPI_CACHE_DIR` 配下の stale `.bin` が `gc --dry-run` の `detail.pypi_cache.would_remove` に現れ、ファイルは削除されないこと）、`gc_removes_stale_pypi_cache_entries`（非 dry-run で `files_removed == 1` かつファイルが削除されること）、`doctor_reports_stale_pypi_cache_count`（`doctor` の `detail.checks` に `pypi_cache` チェックが `stale_count == 1` で現れること）。`cargo test`（全テストパス）、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo fmt -- --check` すべてパス。
- [DONE] PR-UX5: `pybun init -y` default scaffold is buildable by `pybun build` (Issue #260)
  - Goal: `pybun init -y` 直後の既定プロジェクトが hatchling の wheel file selection で失敗せず、quickstart の `init` → `build` が手動編集なしで進められるようにする。
  - Current: `src/cli.rs::InitArgs` の `--template` 既定値を `package` に変更し、明示的な `--template minimal` は従来どおり flat layout の opt-in として維持。既存の `init_project()` が持つ `src/<package>/__init__.py` 生成経路を既定で使うことで、`docs/SPECS.md` の「`-y` は推奨デフォルト設定（src layout 等）を即時適用」に合わせた。
  - Tests: `tests/cli_init.rs` に `init_yes_default_creates_buildable_src_layout` を追加し、`pybun init -y` が `src/demo/__init__.py` を生成することを回帰テスト化。`cargo test --test cli_init`、`cargo test --test cli_init_extra`、`cargo test --test cli_build`、`cargo test --test '*'`、`cargo test`、`cargo fmt -- --check`、`just lint`、`cargo audit`、`cargo deny check licenses` がパス。ローカル環境には `python -m build` / `hatchling` が未導入だったため、実 `pybun build` の外部 backend 実行は未実施。
...
- [DONE] PR-RESOLV1: PEP 508 marker evaluation completeness (Issue #183)
  - Goal: `src/resolver.rs::evaluate_marker()` がサポートする marker 変数を `platform_machine`/`sys_platform`/`platform_system`/`python_version` のみから PEP 508 のフルセットへ拡張し、`in`/`not in` 演算子と引用符を考慮した `and`/`or` 分割を実装する。`extra ==` の扱いを `src/pypi.rs::marker_allows` と一致させる。
  - Current: `evaluate_marker()` を、引用符・括弧を正しく扱うトークナイザ（`tokenize_marker`）と再帰下降パーサー（`MarkerParser` — `marker_or := marker_and ('or' marker_and)*`、`marker_and := marker_expr ('and' marker_expr)*`、括弧によるグルーピング）に置き換え。従来の素朴な `" or "`/`" and "` 文字列分割（引用符内の値を誤分割する TODO）を撤廃。`lookup_marker_variable()` で `os_name`/`sys_platform`/`platform_machine`/`platform_system`/`platform_release`/`platform_version`/`platform_python_implementation`/`implementation_name`/`python_version`/`python_full_version`/`implementation_version` を解決。`os_name`（posix/nt）、`platform_release`/`platform_version`（`uname -r`/`uname -v`、Unix限定・取得失敗時は空文字列）、`platform_python_implementation`/`implementation_name`（`python3 -c "import platform; ..."`、失敗時 "CPython"/"cpython" にフォールバック）、`python_full_version`/`implementation_version`（`python3 --version` のフルバージョン文字列）を新設のヘルパーで取得し `OnceLock` でキャッシュ。`get_python_version()` は `get_python_full_version()` から MAJOR.MINOR を導出するよう再構成（サブプロセス呼び出しを共有）。`in`/`not in` は PEP 508 のとおり「左辺値が右辺値の部分文字列か」で評価。`extra == "..."` / `extra in "..."` は resolver が要求された extras を追跡していないため `false`（非アクティブ扱い）、`!=`/`not in` は `true` を返し、`pypi.rs::marker_allows` の「`extra ==` を含む marker は除外」という挙動と一致させた（`marker_allows` を `pub(crate)`化してクロスモジュールテストで直接比較）。未知の変数・未対応演算子・パース不能な式は従来通り `true`（inclusive、パッケージを静かに除外しない）。Issue #346 のbehavior-preserving extractionで、この実装を `src/markers.rs` へ移動した。
  - Tests: marker characterization tests remain in `src/resolver.rs` and exercise the extracted parser through `Requirement::marker_applies`; resolver, PEP 440 conformance, and property suites preserve the pre-extraction results.
- [DONE] PR-A8: エラーエンベロープの標準化 — 構造化診断コード + hint を全コマンドへ (Issue #191)
  - Goal: `status == "error"` のレスポンスは必ず `diagnostics[]` に `code`（`E_*`）/`message`/`suggestion` を備えたエントリを含むという規約を全コマンドへ徹底し、`detail.error` のような自由形式文字列を主要エラーチャネルにしない。
  - Current: `src/schema.rs::EventCollector` に `error_with_code(code, message, suggestion)` を追加。`src/commands.rs` のトップレベル dispatch（`install`/`add`/`remove`/`lock`/`run`/`x`/`test`/`build`/`mcp serve --stdio`/`gc`/`python {list,install,remove,which}`/`module-find`/`lazy-import`/`watch`/`profile`/`telemetry`/`init`/`outdated`/`upgrade`/`self update`）の汎用フォールバック診断にすべて専用の `E_*_FAILED` コードと再実行ヒントを付与（既存の `pre_error_count` ガードで内部関数が既に詳細診断を積んでいる場合は重複を避ける）。`run_outdated`/`run_upgrade` の lockfile 未検出に `E_LOCKFILE_NOT_FOUND`、`install`/`lock` の sdist-only エラーに `E_INSTALL_SDIST_ONLY`、`run_schema_check`（`collector: &mut EventCollector` を受け取るよう変更）に `E_SCHEMA_VERSION_MISMATCH`/`E_SCHEMA_FILE_READ`/`E_SCHEMA_FILE_PARSE`/`E_SCHEMA_FILE_MISMATCH`、`run_doctor` のサポートバンドル失敗に `E_DOCTOR_BUNDLE_FAILED`、pytest ラッパーのテスト失敗に `E_TEST_FAILED`（suggestion 付き）、watch のネイティブ/ポーリング両ループ失敗に `E_WATCH_LOOP_FAILED`、`self update` のマニフェスト読み込み失敗/アセット未検出/適用失敗に `E_SELF_UPDATE_MANIFEST`/`E_SELF_UPDATE_NO_ASSET`/`E_SELF_UPDATE_APPLY_FAILED` を追加。Issue #191 の例（`pybun outdated` で lockfile 未検出）はそのまま `E_LOCKFILE_NOT_FOUND` + suggestion を返すようになった。
  - Tests: `tests/error_envelope.rs`（新規）— `outdated_without_lockfile_has_coded_diagnostic`/`upgrade_without_lockfile_has_coded_diagnostic`/`remove_without_project_has_coded_diagnostic`/`schema_check_with_unreadable_path_has_coded_diagnostic` が、`status == "error"` のとき `diagnostics[]` が非空かつ各エラー診断に `E_*` の `code`/非空の `message`/非空の `suggestion` が存在することを検証するリグレッションガード。`src/schema.rs` に `test_error_with_code` を追加。`cargo test`（351 lib tests）、`cargo test --test '*'`、`cargo test --test e2e_general`、`cargo fmt`、`cargo clippy --all-targets --all-features -- -D warnings`、`just lint` すべてパス。`docs/SPECS.md` に 5.1.1 エラーエンベロープ規約セクションを追加。
- [DONE] PR-A9: 構造化トレースバック診断 — Python 例外を機械可読 E_* コードへ (Issue #243)
  - Goal: `pybun run` / MCP `pybun_run` で Python プロセスが失敗した際、生の stderr テキストではなく機械可読な診断を `diagnostics[]` に含め、エージェントのループ往復コストを削減する。
  - Current: `src/traceback.rs` を新設。Python traceback を解析して `ParsedTraceback`（コード・例外型・メッセージ・ロケーション・next_action）に変換。ドット記法エラーコード体系（`runtime.module_not_found`、`runtime.syntax_error`、`runtime.type_error` 等 15 種）と `next_action`（`pybun_add` 等のエージェント呼び出し可能な構造化オブジェクト）を実装。MCP `call_run`（`src/mcp.rs`）は exit_code != 0 かつ traceback が検出された場合に `diagnostics[]` を応答 JSON に追加。CLI `run --format=json`（`src/commands/mod.rs`）も同一の traceback パーサーを使って `EventCollector` に構造化診断を追加（MCP/CLI の共通化）。レビュー対応として CLI 診断にも MCP と同じ top-level `exception_type` / `location` / `next_action` を追加し、絶対パス実行時の traceback file を current project 相対へ正規化、`KeyboardInterrupt` など colon なし例外行の解析にも対応。
  - Tests: `src/traceback.rs` 内 25 件のユニットテスト（各例外型・ネストフレーム・SyntaxError 形式・make_relative・絶対パス相対化・bare exception・next_action 抽出等）、`tests/mcp.rs` に 5 件の統合テスト（`ModuleNotFoundError`→diagnostic/next_action/package名、成功時に diagnostic 不在、`SyntaxError`→diagnostic）、`tests/cli_run.rs` に CLI JSON traceback 診断 shape の回帰テストを追加。全テスト・`cargo clippy -D warnings`・`cargo fmt` パス。
- [DONE] PR-A14: MCP structured action audit log (Issue #249)
  - Goal: 全 MCP `tools/call` 呼び出しについて、agent が実際に何を実行したかを parent-owned JSONL audit trail として残し、human review / compliance / CLI-MCP 挙動差分調査に使えるようにする。sandboxed child が自分の監査ログを改ざんできないことを保証する。
  - Current: `src/mcp.rs` に session-scoped `McpAuditLog` を追加し、`session_id`（MCP server session 内で固定）/ `call_id` / timestamp / tool name / input または `input_hash` / tool schema metadata / output summary / sandbox events / `allow_write` 配下の file write diff を JSONL へ追記。既定パスは `PYBUN_HOME` 配下の `logs/mcp-audit-{date}.jsonl`、`PYBUN_AUDIT_LOG` で上書き、`PYBUN_AUDIT_LOG=/dev/null` で無効化。`pyproject.toml` の `[tool.pybun.mcp.audit]` で `path` / `hash_inputs` / `retention_days` を読み、`hash_inputs = true` では raw input を保存しない。`pybun://audit/recent` resource は現 session の直近 20 件を返す。`src/sandbox.rs` の filesystem policy は `open()` だけでなく `os.remove` / `os.rename` / directory mutation などにも適用し、`allow_write` 外の audit log 削除・上書きを `blocked_file_writes` としてブロックする。
  - Tests: `tests/mcp.rs` に 5 件追加 — JSONL が `tools/call` ごとに書かれ同一 `session_id` と tool schema metadata を持つこと、`hash_inputs = true` が raw input を保存しないこと、`pybun://audit/recent` が session entry を返すこと、`PYBUN_AUDIT_LOG=/dev/null` が tool 実行を妨げないこと、sandboxed `pybun_run` が `allow_write` 外の audit log を上書き・削除できず prior entry が残り `file_writes[]` が populated されることを検証。対象テスト: `cargo test --test mcp mcp_audit -- --nocapture`、`cargo test --test mcp mcp_tool_calls_write_jsonl_audit_entries -- --nocapture`。
- [DONE] PR-A16: MCP `pybun_context` — single-call project state snapshot for agents (Issue #245)
  - Goal: エージェントがプロジェクトの状態を把握するために必要な 5〜10 回のツール呼び出しを 1 回に削減する。Python バージョン・venv 状態・lockfile 状態・インストール済みパッケージ・doctor 警告を単一の JSON レスポンスに集約する。
  - Current: `src/mcp.rs` に `call_context()` を実装し、`pybun_context` ツールと `pybun://project/snapshot` リソースを追加。`summary_only: true` でパッケージリストをカウントに縮約（低トークンコンテキスト向け）。`venv_status` は `ok` / `missing` / `corrupt` の 3 値。`lockfile_status` は `in_sync` / `drift` / `missing` の 3 値。`installed_packages` に `declared: bool` / `locked: bool` フィールドを含め、エージェントが問題パッケージを即座に特定できる。`drift_summary.undeclared_imports` は AST drift 機能（Issue #248）実装まで `null`。`snapshot_at_ms` は UNIX エポックミリ秒タイムスタンプ。`doctor_warnings` は `code` / `message` / `severity` の構造化警告を返す。
  - Tests: `tests/mcp.rs` に 7 件追加 — tools/list に pybun_context が含まれること、必須フィールドの存在確認、summary_only モードでカウントフィールドが返されること、空ディレクトリでの venv_status/lockfile_status 確認、resources/list に pybun://project/snapshot が含まれること、resource read でコンテキストデータが返されること。全テスト 46 件パス。`cargo clippy --all-targets --all-features -- -D warnings` / `cargo fmt -- --check` ともにエラーなし。
- [DONE] PR-A15: MCP `pybun_run` sandbox default-on hardening (Issue #244)
  - Goal: MCP 経由の `pybun_run` を安全側に倒し、agent が `sandbox_policy` を省略しても sandbox を有効化する。sandbox 無効化は `unsafe_no_sandbox: true` の明示指定に限定し、human approval 用の `dry_run` と追加 resource/env 制限も提供する。
  - Current: `src/mcp.rs::call_run()` は `sandbox_policy` 省略時も `SandboxConfig::default()` で sandbox を適用し、`unsafe_no_sandbox: true` の場合のみ無効化して `W_MCP_UNSAFE_NO_SANDBOX` warning を返す。`dry_run: true` は実行せず `would_execute` / `sandbox_policy` / `risk_level` / `risk_reasons` を返す。`src/sandbox.rs` は `RLIMIT_NPROC=0` と `RLIMIT_FSIZE=10MB` を sandbox の既定 resource limit に追加し、`allow_env` から `AWS_*` / `*_SECRET*` / `*_TOKEN` / `*_KEY` 形式の credential 名を拒否する。sandbox audit file は helper tempdir とは別の parent-owned tempdir に作成し、sitecustomize の filesystem policy で通常の Python 書き込み・削除対象から除外する。
  - Tests: `tests/mcp.rs` に回帰テストを追加 — policy 省略時の subprocess block / `unsafe_no_sandbox` opt-out warning / `dry_run` 非実行 / credential `allow_env` 拒否 / sandbox audit file 書き込みブロック / `allow_network` policy 維持 / resource limit JSON (`max_processes=0`, `file_size_limit_mb=10`)。`tests/sandbox.rs` に `sandbox_allow_env_rejects_credential_like_names` を追加し、既存 allow_env テストは非credential名へ更新。検証: `cargo test --test mcp`、`cargo test --test sandbox`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo fmt -- --check`。
- [DONE] PR-A17: MCP `pybun_test` — structured per-test JSON output and `--changed` flag (Issue #246)
  - Goal: エージェントのテストワークフローを効率化するため、`pybun_test` MCP ツールを追加し、テスト結果を構造化 JSON（summary/failures/passed）で返す。`changed: true` で git 変更ファイルに絞ったテスト実行をサポート。
  - Current: `src/mcp.rs` に `call_test()` を実装し、`pybun_test` ツールと `get_changed_test_files()` ヘルパーを追加。`TestDiscovery` + `TestExecutor` を MCP から直接呼び出し（CLI 経由ではなく Rust 内部 API 再利用）。`changed: true` は `git diff --name-only HEAD` と `git ls-files --others --exclude-standard` の両出力から変更 .py ファイルを収集し、テストファイルは直接、ソースファイルは `src/foo.py → tests/test_foo.py` 規約でマッピング。`failures[].rerun_command` は `pybun test <file>::<name>` 形式。`filter` パラメータによるテスト名フィルタリングもサポート。
  - Tests: `tests/mcp.rs` に 6 件追加 — `mcp_tools_list_includes_pybun_test`（tools/list に pybun_test が含まれること）、`mcp_pybun_test_all_passing_returns_summary`（summary/failures/passed の構造と型を確認）、`mcp_pybun_test_failures_include_rerun_command`（failures に name/file/line/duration_ms/status/rerun_command が含まれること）、`mcp_pybun_test_passed_entries_have_required_fields`（passed エントリの必須フィールドと status="passed" を確認）、`mcp_pybun_test_filter_runs_matching_tests`（filter="alpha" で 1 件のみ実行）、`mcp_pybun_test_empty_project_returns_empty_summary`（テストファイルなしで total=0）、`mcp_pybun_test_changed_runs_git_modified_files`（git init 済みリポジトリで committed ファイルを除く変更ファイルのみ実行）。`cargo clippy --all-targets --all-features -- -D warnings` / `cargo fmt -- --check` ともにエラーなし。
- [DONE] PR-A18: sandbox `--allow-env` の credential 拒否をサイレントから警告診断へ (Issue #259)
  - Goal: `--allow-env=<VAR>` で明示的に allow-list した変数名が credential 形状（`AWS_*` / `*_SECRET*` / `*_TOKEN` / `*_KEY`）だった場合、`is_credential_env_name` により default-deny のまま正しくブロックされる（これはセキュリティ上意図した挙動であり変更しない）が、これまで `detail.sandbox.allow_env` が黙って `[]` になるだけで、ユーザーがなぜ自分の指定が無視されたのか診断する手段がなかった。これを「サイレントな no-op」に見えないよう、拒否時に明示的な warning 診断を返すようにする。
  - Current: `src/sandbox.rs::SandboxGuard` に `rejected_env: Vec<String>` フィールドを追加し、`apply_python_sandbox()` で `config.allow_env` を `is_credential_env_name` で allow/reject に振り分けて保持する。`src/commands/mod.rs` に `emit_rejected_allow_env_diagnostics()` を追加し、`run_script` / inline `-c` 実行の両方の sandbox 経路で `guard.rejected_env` の各名について `W_SANDBOX_ALLOW_ENV_REJECTED` warning 診断（理由込みメッセージ）を `events`/`diagnostics` 配列に出力する。env フィルタ自体の挙動（credential 名は allow-list されても絶対に子プロセスへ渡さない）は変更していない。
  - Tests: `src/sandbox.rs` に単体テスト3件を追加 — `apply_python_sandbox_passes_through_non_credential_allow_env`、`apply_python_sandbox_rejects_credential_shaped_allow_env`、`apply_python_sandbox_splits_mixed_allow_env_into_allowed_and_rejected`（allow_env/rejected_env の分離を検証）。`tests/sandbox.rs` に `sandbox_allow_env_rejection_surfaces_warning_diagnostic` を追加し、JSON 出力に `W_SANDBOX_ALLOW_ENV_REJECTED` と対象変数名が含まれることを確認。検証: `cargo test --lib sandbox::`、`cargo test --test sandbox`、`cargo test --test mcp`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo fmt -- --check`。
- [DONE] PR-A19: pip シム bootstrap のチェックサム検証をフェイルオープンからフェイルクローズへ (Issue #283)
  - Goal: `pip install pybun-cli` / pipx がバイナリの信頼起点として使う `pybun/bootstrap.py` が、マニフェストの `sha256` が `"placeholder"` の場合に検証を黙ってスキップしていた（Rust 側 `self_update.rs::verify_checksum` は同条件を明示的に拒否しており非対称だった）。あわせて `http://` アセット URL の許容と、tar 展開時の symlink/hardlink 経由のディレクトリ脱出未対策も是正する。
  - Current: `pybun/bootstrap.py::_verify_checksum` から placeholder サイレントスキップを削除し、`self_update.rs` と同様に `BootstrapError` を送出するよう変更。`_download_url` は `https://`（および開発用 `file://`）のみを許可し、`http://` は明示エラーで拒否。`_safe_extract_tar` は各メンバーのパス脱出チェックに加え、symlink/hardlink の `linkname` も展開先配下に収まっているか検証し、Python 3.12+ では `tarfile.extractall(..., filter="data")` を併用。`PYBUN_PYPI_NO_VERIFY=1` で検証をスキップする既存の抜け道はそのまま維持しつつ、`ensure_binary()` 内で stderr に大きな警告を出力するようにした。
  - Tests: `pybun/tests/test_bootstrap.py` に16件（既存7件 + 新規9件）。`VerifyChecksumTests`（placeholder 拒否・空/欠落拒否・正しいハッシュ許可・不一致拒否）、`DownloadUrlSchemeTests`（`http://` 拒否・`file://` 許可）、`SafeExtractTarTests`（パストラバーサル拒否・symlink 脱出拒否・正常展開）。`pytest pybun/tests/test_bootstrap.py` 16 passed。既存の Rust 統合テスト `cargo test --test pypi_shim`（`file://` + 実 sha256 を使用）も引き続きパスすることを確認。
- [DONE] feat: `pybun audit` — CLI exposure of OSV vulnerability scanning (Issue #316)
  - Goal: `pybun_audit`（MCP tool, Issue #247）は OSV ベースの脆弱性スキャンを提供していたが CLI には存在せず、MCP JSON-RPC を話さない human/CI から実行できなかった。PR-A3 の「MCP/CLI の実処理統一」方針に沿い、OSV クエリ・重大度正規化・fix version 抽出ロジックを共有モジュールへ切り出した上で CLI コマンドとして公開する。
  - Current: 新設の `src/audit.rs` に `list_installed_packages()`（`pip list --format=json` 経由でインストール済みパッケージを列挙）・`scan_for_vulnerabilities()`（OSV `querybatch` へ問い合わせ、`severity_threshold` 以上の脆弱性のみ `AuditReport` として返す）・重大度正規化ヘルパー群（`severity_level`/CVSS v3 ベクタ解析）を実装し、`src/mcp.rs::call_audit` と新設の CLI コマンドの双方がこの共有ロジックに委譲するよう変更（`mcp.rs` 内に重複定義されていた `audit_*` ヘルパー関数群は削除）。`src/cli.rs` に `Commands::Audit(AuditArgs)`（`--severity-threshold <low|medium|high|critical>` 既定 `low`、`--fail-on <LEVEL>` で CI gating 用に非ゼロ終了）を追加し、`src/commands/maintenance.rs::run_audit()` が `--format=json` の `diagnostics[]` に脆弱性ごとの `W_AUDIT_VULNERABILITY_FOUND` warning（fix version があれば `suggestion` に `pybun upgrade` を提案）を出力する。`--fail-on` 閾値以上の脆弱性が見つかった場合は `RenderDetail::error()` を返し、プロセスも非ゼロ終了する（既存の `is_error` → `std::process::exit(1)` 経路を再利用）。ネットワーク I/O を伴うため `src/entry.rs::requires_tokio_runtime()` に `Commands::Audit(_)` を追加。
  - Tests: `src/audit.rs` にユニットテスト10件（severity ランキング・CVSS v3 ベクタ解析・fix version 抽出・空パッケージリストのショートサーキット等）。この過程で、既存の CVSS ベクタパーサに実在した未検出バグ（`get("C")` が `starts_with` で先頭の `"CVSS:3.1"` セグメントに誤マッチし、`C:H` を読み取れず severity が `medium` に落ちる）を発見し修正（exact-key match へ変更）。`tests/cli_audit.rs` に E2E 6件を新設 — mocked OSV での空脆弱性/1件検出、`--fail-on` が閾値以上で非ゼロ終了・閾値未満では成功終了することの両方、`--severity-threshold` によるフィルタ、`--help` のフラグ記載確認。`tests/mcp.rs` の既存 `pybun_audit` テスト（3件）は無変更のまま全パスを確認（共有ロジックへの委譲後も MCP 応答 JSON 形状は変更なし）。`tests/compat_snapshots.rs` に `help_audit` を追加し `help_root`/`help_audit` スナップショットを更新。全テスト(1022件)・`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` がパス。
  - Review follow-ups (Codex 第三者レビュー指摘を解消, PR #336): (1) **[High]** `--severity-threshold` が OSV クエリ段階のフィルタとして働くため、`--severity-threshold=critical --fail-on=high` のように `--fail-on` がより低い重大度を指定した場合、その脆弱性が `--fail-on` 判定前に握りつぶされ CI gate が黙って通過してしまう不具合を修正。`run_audit()` は「表示用（`--severity-threshold`）」と「CI gate 判定用（`--fail-on` があればその閾値との min）」で異なる有効閾値を使い分け、OSV へは常により許容的な（数値の低い）閾値でスキャンし、`--fail-on` 判定は非表示フィルタ前の完全な結果集合に対して行うよう変更。表示・集計（`vulnerabilities`/`summary` の各カウント/`W_AUDIT_VULNERABILITY_FOUND` 診断）は引き続き `--severity-threshold` でフィルタする。(2) **[High]** `list_installed_packages()` が pip 起動失敗・非ゼロ終了・不正な JSON 出力のいずれも黙って空リストへフォールバックしていたため、`pybun audit --fail-on=...` は「壊れた環境をスキャンできなかった」場合でも「0パッケージをスキャンして成功」という偽陽性の成功終了を返し得た。`list_installed_packages()` の戻り値を `Result<Vec<InstalledPackage>, String>` に変更し、CLI 側（`run_audit()`）は `find_python_env()`/`list_installed_packages()` の失敗を `E_AUDIT_PYTHON_ENV_NOT_FOUND`/`E_AUDIT_PIP_LIST_FAILED` エラー診断として明示し非ゼロ終了するよう修正（MCP 側 `call_audit` は既存の agent 向け fail-open 挙動を意図的に維持するため `unwrap_or_default()` で従来通り空リスト継続とし、空パッケージ時の応答 JSON も `unscanned` キーを含まない従来形状へ復元）。(3) **[Medium]** `--fail-on` 閾値超過時に `RenderDetail::error()` で JSON envelope status を `"error"`／プロセスを非ゼロ終了させていたが、対応する `error` レベル診断が存在せず `warning` の `W_AUDIT_VULNERABILITY_FOUND` のみだった（`CLAUDE.md` の「エラーは diagnostics に kind/message/hint 付きで記録する」規約と不整合）ため、閾値超過時に `E_AUDIT_FAIL_ON_THRESHOLD` エラー診断（該当件数・`--fail-on` 値・remediation hint 付き）を追加出力するよう修正。(4) **[Medium]** CVSS ベクタパーサが `"CVSS:"` prefix のみでヘッダ検証していたため、CVSS v2（バージョンセグメントなし）や将来の CVSS v4 ベクタも v3 のメトリクス意味論で誤分類され得た問題を修正し、`"CVSS:3.0/"`/`"CVSS:3.1/"` の完全一致のみ受理するよう厳格化。(5) **[Low]** MCP `pybun_audit` の空パッケージレスポンス JSON に共有モジュール委譲後は常に `"unscanned": 0` が含まれるようになり、リファクタ前（キー自体が存在しない）から shape が変わっていた点を、空パッケージケースを委譲前に早期リターンする形で復元し解消。
  - Tests (follow-up): `src/audit.rs` に単体テスト6件追加 — CVSS v3 メトリクス順序非依存の確認、CVSS v2/malformed（v4様式）ベクタの拒否、`list_installed_packages` が存在しない python パスに対し `Err` を返すこと、`severity_str_for_level`/`severity_level` の往復変換。`tests/cli_audit.rs` に E2E 2件追加 — `audit_fail_on_still_triggers_when_below_severity_threshold`（`--severity-threshold=critical --fail-on=high` で high 脆弱性が非ゼロ終了・`E_AUDIT_FAIL_ON_THRESHOLD` 診断を伴うことを確認する回帰テスト）、`audit_reports_error_when_pip_list_fails_instead_of_silently_passing`（pip list が失敗する fake venv で非ゼロ終了・`E_AUDIT_PIP_LIST_FAILED` 診断を確認）。既存 `tests/mcp.rs` の `pybun_audit` テスト（3件）は無変更のまま全パス。全テスト(1029件)・`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` がパス。
  - Follow-up (Issue #338): CLI `pybun audit` は `find_python_env()` が project venv / `PYBUN_ENV` / `PYBUN_PYTHON` / `.python-version` のいずれも見つけられない場合に `EnvSource::System`（host global site-packages）へサイレントにフォールバックし、host のパッケージを誤ってスキャンしていた（`pybun install` を #286/#289 で修正したのと同種のバグ）。`src/commands/maintenance.rs::run_audit()` に `--system` フラグ（`src/cli.rs::AuditArgs`）を追加し、`find_python_env()` の結果が `EnvSource::System` の場合は既定で `E_AUDIT_NO_PROJECT_ENV` エラー診断を出して拒否（`pybun install` の実行または `PYBUN_ENV` 設定、もしくは明示的な `--system` を促す）、`--system` 指定時のみ `W_AUDIT_SYSTEM_ENV` warning 診断 + stderr warning を出したうえでスキャンを継続するよう変更。あわせて、成功・エラーいずれの JSON `detail` にも `python_env: { path, version, source }` を常に含め、実際に（あるいは拒否された場合に何が）スキャンされたかを disclose する。MCP 側 `call_audit`（独立実装）は本 PR のスコープ外（PR-A3 で追って統一）。
    - Tests: `tests/cli_audit.rs` に3件追加 — `audit_refuses_system_python_fallback_without_flag`（venv 無しの tempdir で `--system` なしの場合に非ゼロ終了・`E_AUDIT_NO_PROJECT_ENV` 診断・`detail.python_env` が `"system PATH (GLOBAL)"` を示すことを確認）、`audit_system_flag_allows_fallback_with_warning`（`--system` 指定時は成功終了しつつ `W_AUDIT_SYSTEM_ENV` warning 診断と stderr warning を確認）。既存テストのうち venv なしで実行していた2件（`audit_json_returns_valid_structure_with_mocked_osv`・`audit_severity_threshold_filters_low_severity_findings`）は `--system` を付与するよう更新し、`detail.python_env` のアサーションを追加。`PYBUN_ENV` で明示 venv を指定する既存テスト（`audit_json_reports_vulnerability_from_mocked_osv`）にも `detail.python_env.source == "PYBUN_ENV (LOCAL)"` の確認を追加。`tests/snapshots/compat/help_audit.txt` を `--system` フラグ追加に合わせて再生成。全テスト・`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` がパス。
...
- PR-A3: MCP と CLI の実処理統一
  ...
  - [DONE (partial: pybun_install only)] Issue #284: `pybun_install` が実際には wheel をダウンロード/インストールせず、常に `"status": "installed"` / `"Resolved and installed N packages"` を返していた（fixture-only index 探索・`sha256:placeholder` の捏造・架空のホイールファイル名・`offline` 引数の無視を含む）。
    - Current: `src/mcp.rs::call_install` を書き換え、CLI `pybun install` と同一の `crate::commands::install()` に完全委譲するように変更。これにより実際の index 選択（`index` 未指定時は PyPI フォールバック、CLI と同じ）、実際の sha256 検証（placeholder 拒否）、実際の wheel ダウンロード/インストール、`offline`/`system` 引数の反映が一括で実現された。`InstallOutcome` に `installed_count`（実際にダウンロード＆インストールされた wheel 数）を追加し、`pybun_install` は `installed_count > 0` の場合のみ `"status": "installed"` を報告、それ以外は誠実に `"status": "resolved"`（依存解決・lockfile 生成のみで wheel は未インストール）を返す。副次効果として lockfile 名も CLI と同じ `pybun.lockb` になった（本 Issue のスコープ内での自然な帰結であり、A3 全体の統一はまだ未完了）。
    - Tests: `tests/mcp.rs` に3件追加 — `mcp_tools_call_install_reports_resolved_not_installed_when_no_wheel_downloaded`（wheel URL を持たない index では `"resolved"` を返し `installed_count: 0`、real hash を検証）、`mcp_tools_call_install_actually_installs_wheel_and_reports_honest_status`（httpmock 経由の real wheel install で `"installed"` を返し `installed_count > 0`）、`mcp_tools_call_install_honors_offline_flag`（空キャッシュ + `offline: true` でネットワークに到達せず誠実に失敗）。`cargo test`（全50スイート）・`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` すべてパス。
    - Note: `pybun_run`/`pybun_resolve` など他ツールの CLI 実装への統合は引き続き A3 本体のスコープとして未完了。
  - [DONE] Issue #272: `mcp.rs::call_run` が Python interpreter discovery・sandbox 設定・実行/タイムアウト処理を CLI `pybun run`（`commands::run_script` / `run_python_code`）から独立して再実装しており、将来の CLI 側の挙動変更が MCP 側へ自動的に反映されない divergence risk があった。
    - Current (Phase 1): interpreter discovery（`find_python_env()`）は元々 MCP・CLI 双方が同一関数を呼んでおり分岐していなかったことを確認。実際に分岐していたのは「サンドボックス適用後のコマンド実行（sandboxed/plain の分岐・タイムアウト処理・stdout/stderr capture）」で、これが `run_script`・`run_python_code`・`mcp::call_run` の3箇所にほぼ同一のロジックとして重複していた。`src/sandbox.rs` に共有プリミティブ `execute_with_optional_sandbox(cmd, sandbox_guard: Option<&SandboxGuard>, capture_output: bool) -> Result<SandboxedExecution>` を新設し、3箇所すべてをこの関数へ委譲するよう置き換え。`SandboxConfig` の構築（CLI フラグ vs MCP `sandbox_policy` JSON）と応答フォーマットは各呼び出し側の責務のまま維持（入力形状が異なるため統一不可能かつ非目標）。CLI `pybun run` の外部から観測可能な挙動（sandboxed/plain 実行結果・タイムアウト・出力）は変更なし。MCP `pybun_run` の応答 JSON 形状（`sandboxed`/`audit`/`resource_limits`/`dry_run` 等）も変更なし。
    - Current (Phase 2, 完了): 残っていた divergence — MCP `pybun_run` が CLI `run_script` の PEP 723 自動インストール（依存解析・キャッシュ済み venv 再利用・resolve/install・backend 選択）を一切実行せず、`# /// script` メタデータを持つスクリプトを渡すと単にシステム Python でそのまま実行して import エラーになっていた — を解消。`mcp.rs::call_run` を `async fn` に変更し、`crate::cli::RunArgs` を組み立てたうえで CLI と同一の `crate::commands::run_script(&args, &mut collector, OutputFormat::Json)` に完全委譲するよう書き換え（`call_install` が `commands::install()` に委譲する既存パターンを踏襲）。`RunOutcome`/`RunProfileInfo`/`SandboxInfo`/`run_script` を `pub(crate)` へ変更し MCP から参照可能にした。`OutputFormat::Json` を渡すことで stdout/stderr capture を強制し Unix 専用の `cmd.exec()` 置換バイパスを回避（MCP のリクエスト/レスポンスモデルに必須）。CLI 側の「失敗時に stderr から traceback を構造化 diagnostic へ変換する」ロジックを `call_run` にも複製し、`run_script` 内部で発生する sandbox 関連の diagnostics/warnings（`W_SANDBOX_LIMIT_UNSUPPORTED` 等）も `EventCollector` 経由で応答へ反映されるようになった（旧実装は `EventCollector` を一切使わずこれらを黙って破棄していたため、これは純粋な改善）。MCP 独自の `dry_run`（risk-preview）と CLI/MCP 共有の `PYBUN_PEP723_DRY_RUN`（テスト用の疑似install フラグ）は別概念のまま維持し混同していない。
    - Tests (Phase 1): `src/sandbox.rs` の `tests` モジュールに5件追加 — plain status/output capture、Linux sandbox timeout、sandbox output capture、raw `std::io::Error` preservation をUnix/Windows別のcommand fixtureで検証。
    - Tests (Phase 2): `tests/mcp.rs` に `mcp_pybun_run_auto_installs_pep723_dependencies` を追加（Red→Green で確認）— `PYBUN_PEP723_DRY_RUN=1` 環境下で `# /// script` 付きスクリプトを `pybun_run` に渡し、応答 JSON の `pep723_dependencies`/`pep723_backend`/`temp_env` が CLI `pybun run` 相当の値を持つことを検証。全テスト（`cargo test`: 単体513件・各 integration スイート含め全パス、`cargo test --test mcp`: 68件パス、`cargo test --test cli_run`: 41件パス）・`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` すべてパス（ローカル環境の壊れた `.venv`（存在しない Homebrew Python 3.14 framework 参照）に起因する無関係な事前失敗を `PYBUN_PYTHON` override で個別に切り分け・確認済み）。
    - Current (Issue #346 shared resolve seam): `src/resolve_service.rs` に transport-independent な `ResolveRequest` / `resolve()` 境界を追加し、CLI の install/lock/outdated/upgrade/PEP 723 run と MCP `pybun_resolve` の全 resolve 呼び出しを同じ service 経由へ統一。index discovery と CLI/MCP 固有の response rendering は各 adapter に残し、resolver core API、CLI output、JSON schema v1、MCP wire、lockfile/install semantics は変更しない。
    - Tests: shared service の resolver delegation / `ResolveOptions` 伝播 unit tests に加え、CLI install/lock/run と MCP integration suites で既存 contract を回帰確認する。
    - Current (Issue #346 dispatcher seam): 905行規模だった `execute()` の command-specific match を `src/commands/dispatch.rs` へ抽出し、package/execution/testing/tooling/maintenance/python/project の focused dispatch functions に分割。`execute()` は progress/event lifecycle、group routing、共通 render、exit-code propagation の orchestration のみに縮小し、`src/commands/mod.rs` は1,503行から576行になった。各 command arm の処理順・diagnostic・detail構築は移動のみで維持する。
    - Tests: `tests/commands_decomposition.rs` が `commands/mod.rs` の600行上限、dispatch entry point、7つの focused dispatcher を固定し、`cli_smoke`/`json_schema`/`mcp`/`e2e_general` suites が text/JSON/MCP/command behaviorをcharacterizeする。
...
- [IN PROGRESS] PR-A5: 依存入力範囲の拡張（`optional-dependencies`/dependency groups/workspace member globs）(Issue #119)
  ...
  - Follow-up (Issue #285, honest-degradation warning only — full extras resolution remains open/未着手): PEP 508 extras（例: `typer[all]`）が `Requirement::from_str` の名前解析に紛れ込み、PyPI メタデータ取得URLを壊す（例 `pypi/typer[all]/json` への404）か、ブラケットを含む偽の名前で解決が失敗するかのいずれかで、ユーザーの意図（extra の依存関係）が診断なしに黙って無視されていた（Issue #93 の場当たり的な fix の副作用）。`Requirement` に `pub extras: Vec<String>` フィールドを追加し、`FromStr` で `[...]` セグメントを名前から正しく分離・記録するようにした上で、`src/commands/mod.rs::warn_on_ignored_extras()` が `install()`（`--require` 経由 / `pyproject.toml` 経由の両方）で解析された `Requirement` を走査し、`extras` が非空なら `W_EXTRAS_IGNORED` の warning 診断（対象パッケージ名・落とされた extras 一覧を含む）を `--format=json` の `diagnostics[]` と人間可読出力（`eprintln!`）の両方に出す。`pybun add` は常に内部で `install()` を呼び出すため、同じ警告経路で自動的にカバーされる（二重発火を避けるため `add_package()` 自体では警告しない）。extras 自体の依存関係解決・インストールは引き続き未実装（本PRのスコープ外、PR-A5 本体で扱う）。
    - Tests: `src/resolver.rs` に単体テスト5件（`test_parse_requirement_with_single_extra` 等 — 単一/複数 extras、バージョン制約併記、marker併記、extras無しで空Vec）。`tests/json_output.rs` に統合テスト1件（`install_with_extras_outputs_w_extras_ignored_diagnostic` — オフラインfixtureインデックスで `app[extra1,extra2]==1.0.0` を解決し、ベースパッケージ `app` は正常にインストールされつつ `W_EXTRAS_IGNORED` 診断が `context.package`/`context.extras` を含めて出ることを確認）。全テスト(977件)・`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` がパス。
...
- [DONE] fix: sandbox `--sandbox-cpu` kill diagnostic + `resource_limits.unsupported` surfacing (Issue #203, follow-up to #152)
  - Goal: PR-SEC3 (#152) で `--sandbox-timeout` は `E_SANDBOX_TIMEOUT` 診断を出すが、`--sandbox-cpu` による SIGXCPU kill には対応する `E_*` 診断がなく、また `detail.sandbox.resource_limits.unsupported`（例: macOS での memory）が `diagnostics[]` に warning として現れず、#191 の標準エラーエンベロープと非一致だった。これら2点のギャップを埋める。
  - Current: `src/sandbox.rs` に `cpu_limit_exceeded(status: &ExitStatus) -> bool` を追加（Unix では `ExitStatusExt::signal() == Some(libc::SIGXCPU)` を判定、非Unixでは常に `false`）。`src/commands/mod.rs` に共通ヘルパー `emit_unsupported_resource_limit_diagnostics()` を追加し、`apply_python_sandbox()` 直後に `resource_limits.unsupported` の各エントリ（例: `"memory"`）について `W_SANDBOX_LIMIT_UNSUPPORTED` の `warning` 診断（"sandbox {limit} limit is not enforced on this platform and will have no effect"）を `run_script`/`run_python_code` の両経路で出力。実行後、`timed_out` でない場合に `args.sandbox_cpu > 0 && sandbox::cpu_limit_exceeded(&status)` を満たせば `E_SANDBOX_CPU_LIMIT` の `error` 診断（"sandboxed process killed after exceeding --sandbox-cpu={N}s of CPU time"、`--sandbox-cpu` 増加/無効化や最適化を促す `suggestion` 付き）を両経路で出力（既存の `E_SANDBOX_TIMEOUT` と同じ UX パターン）。
  - Tests: `src/sandbox.rs` にユニットテスト2件追加（`cpu_limit_exceeded_detects_sigxcpu`・`cpu_limit_exceeded_ignores_other_signals_and_normal_exit`、Unix限定）。`tests/sandbox.rs` に2件追加（`sandbox_cpu_limit_kill_emits_diagnostic` — Unix限定、`--sandbox-cpu=1` のbusy loop killで `diagnostics[]` に `"code":"E_SANDBOX_CPU_LIMIT"` / `"level":"error"` / `--sandbox-cpu` を含むことを確認、`sandbox_memory_limit_unsupported_emits_warning_diagnostic` — macOS限定、`--sandbox-memory` 指定時に `diagnostics[]` に `"code":"W_SANDBOX_LIMIT_UNSUPPORTED"` / `"level":"warning"` が現れることを確認）。全sandboxテスト(31件)・`cargo test --release`(フルスイート)・`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` がパス。
...
- [DONE] PR-A7: install の安全な既定ターゲット（プロジェクト隔離環境） (Issue #286)
  - Goal: `pybun install` が既存のvenv/`PYBUN_ENV`を検出できない場合に、警告のみでsystem Pythonのsite-packagesへ直接インストールしていた fail-open 挙動を解消する。既定では `.pybun/venv` を自動作成してそこへインストールし、system Pythonへのインストールは明示的な `--system` フラグを要求する（uv類似のセーフガード）。
  - Current: `src/env.rs` に `create_project_venv(project_root)`（既存の `.pybun/venv` を再利用、無ければ `find_system_python()` で見つけたインタプリタから `python -m venv` で作成）と `externally_managed_marker(python_path)`（PEP 668 `EXTERNALLY-MANAGED` マーカーをsysconfig経由で検出）を追加。`src/cli.rs::InstallArgs` に `--system` フラグを追加。`src/commands/mod.rs::install()` のwheelインストール経路で、解決済み環境が `EnvSource::System` の場合: (1) `--system` 指定時はマーカーが存在すれば `E_INSTALL_EXTERNALLY_MANAGED` エラーで拒否、無ければ警告を出してそのままインストール、(2) `--system` 未指定時は `.pybun/venv` を自動作成してそちらへインストール先を切り替える。`pybun add`（内部で`install()`を呼び出す)は `system: false` の既定を維持。
  - Follow-up (Issue #289, [DONE]): `.python-version` 経由の解決が、pyenv非管理のbare PATH lookup（`which python3.X`/`python3`）にフォールバックした場合、`EnvSource::PythonVersionFile` タグのため上記ガードを素通りしてしまう抜け穴を修正。`src/env.rs::find_python_for_version()` の戻り値を `(PathBuf, bool)`（`is_pyenv_isolated`）に変更し、`find_python_env()` でpyenv非経由（bare PATH解決）の場合は `EnvSource::System` としてタグ付けするよう変更（pyenv経由の場合のみ従来通り `PythonVersionFile` を維持）。これにより `install()` の既存ガードが自動的に適用される。
  - Tests: `src/env.rs` にユニットテスト1件追加（`python_version_file_bare_path_lookup_is_tagged_as_system` — pyenv不在環境で `.python-version` がシステムPythonのバージョンに解決される場合に `EnvSource::System` としてタグ付けされることを確認。pyenv検出時はスキップ）。`cargo test --lib`, `cargo test --test cli_install --test pypi_integration --test compat_snapshots --test cli_add_remove --test json_schema`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo fmt -- --check`。
  - Tests: `src/env.rs` にユニットテスト4件（`create_project_venv_creates_pybun_venv_directory`、`create_project_venv_reuses_existing_venv`、`externally_managed_marker_detects_pep668_file`、`externally_managed_marker_absent_when_no_marker_file`）。`tests/pypi_integration.rs` にE2E2件（`install_creates_project_local_venv_instead_of_system_python_by_default` — `PYBUN_ENV`未設定・venv無しの新規プロジェクトで`pybun install`が`.pybun/venv`を自動作成しそこにのみwheelをインストールすること、PATH経由で解決される"system"インタプリタ側へは漏れないことを確認、`install_system_flag_installs_into_resolved_system_python` — `--system`指定時はPATH経由のsystemインタプリタへ実際にインストールされ、警告メッセージが出力され、`.pybun/venv`が作成されないことを確認）。`cargo test`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo fmt -- --check`。
...
- [DONE] PR-UX4: `pybun x` propagates child process exit code (Issue #185)
  - Goal: `pybun x <pkg>` が実行したツールの非ゼロ終了コードを `pybun run`（Issue #148/#155）と同様にシェルへ伝播する。これまでは `XOutcome.exit_code` が JSON detail に出力されるのみで `RenderDetail::with_process_exit_code()` に配線されておらず、常に exit code 0 を返していた。
...
- [IN PROGRESS] PR-A10: `src/commands.rs` を per-command モジュールへ分割 (Issue #186)
  ...
  - Current（Issue #271 follow-up）: `pybun add` / `pybun remove` / `pybun install` / `pybun outdated` / `pybun upgrade` の実装を `src/commands/install.rs` へ抽出し、`mod install;` を宣言。`mod.rs` 側の dispatcher を `install::install(...)` / `install::add_package(...)` / `install::remove_package(...)` / `install::lock_dependencies(...)` / `install::run_outdated(...)` / `install::run_upgrade(...)` / `install::warn_on_prerelease_fallback(...)` 呼び出しへ更新。`install.rs` 内の項目は `pub(super)` を既定とし、`mcp.rs` など外部から直接参照される `install` 関数・`InstallOutcome` / `RunOutcome` とそのフィールドのみ `pub(crate)` 化。`mod.rs` からの再エクスポートは `pub(crate) use install::{InstallOutcome, RunOutcome, install};` に限定し、他の型は `mod.rs` 内 private use に留める。JSON detail / diagnostics / summary は変更なし。`mod.rs` は 4903行 → 3798行に縮小、新設の `install.rs` は 1662行。
  - Tests: `cargo test --test '*'`（`cli_run` の一部失敗はローカル環境の壊れた Homebrew `python@3.14` に起因する既存の環境依存問題であり、`git stash` で本変更を除いた HEAD でも同一の失敗が再現することを確認済みのため本PRのスコープ外）/ `cargo test --test e2e_general`（9 passed）/ `just lint`（clippy + fmt check、warning なし）/ `cargo fmt -- --check`（クリーン）。
  - Remaining: `lock.rs` / `run.rs` / `build.rs` / `x.rs` / `python_env.rs` / `project.rs`（init/outdated/upgrade）への抽出は follow-up PR で対応する。
- [DONE] PR-A11: production `unwrap()` / `expect()` panic reduction pass (Issue #187)
  - Goal: 非テストの本番経路に残る panic を減らし、recoverable な状態を JSON diagnostics / Result 経路へ戻す。特に PyPI cache / verification / watcher / snapshot / module scan の周辺で、agent-facing な実行が unstructured panic で落ちないようにする。
  - Current: `src/pypi.rs` の stale cache notice Mutex が poison された場合でも `take_stale_cache_notices()` / stale notice push が panic せず `into_inner()` で復旧するよう変更し、`PyPiIndex::get()` の `cached.expect("checked")` を `Option` 分岐に置換。`src/commands/mod.rs::ensure_selection_is_verifiable()` の strict verification 後 hash 取得を `expect` から `E_VERIFY_MISSING_HASH` diagnostic 付き `Err` に変更。`src/commands/tooling.rs::run_watch()` の target unwrap、`src/module_finder.rs` の suffix unwrap、`src/snapshot.rs::load_for()` の HashMap get unwrap、`src/schema.rs::EventCollector::event()` の `last_mut().unwrap()`、`src/downloader.rs::Downloader::new()` の reqwest client build expect を panic しない実装へ置換。
  - Tests: TDD regression として `pypi::tests::stale_cache_notice_drain_recovers_from_poisoned_mutex` を追加し、poisoned Mutex でも stale cache notice を drain できることを検証。関連検証: `cargo test --lib pypi::tests::stale_cache_notice_drain_recovers_from_poisoned_mutex`（Red → Green）、`cargo test --lib pypi::tests::`、`cargo test --lib snapshot::tests::`、`cargo test --lib schema::tests::test_event_collector`、`cargo test --lib module_finder::tests::test_scan_directory`、`cargo test --test hot_reload test_watch`。
- [DONE] PR-A13: self-healing diagnostics — structured fix plans and `doctor --fix` (Issue #118)
  - Goal: Issue #118 が提案する self-healing diagnostics のうち、`pybun doctor` に絞った実用的な第一段（`install`/`run`/`build`/`test` への `--explain`/`--plan` 展開や MCP 露出は将来 PR のスコープとする）を実装する。診断に機械可読な remediation candidate を添付し、低リスクなものは `--apply` で自動修復できるようにする。
  - Current: `src/schema.rs` に `RiskLevel`（Low/Medium/High）と `FixCandidate { command, description, risk, auto_applicable }` を追加し、`Diagnostic` に `fix_candidates: Option<Vec<FixCandidate>>` フィールドと `with_fix_candidates()` ビルダーを追加（`schema/schema_v1.json` の `diagnostic`/`fix_candidate` 定義も同期）。`src/self_heal.rs` に `fix_candidates_for_missing_python()` / `fix_candidates_for_stale_pypi_cache()` / `fix_candidates_for_lock_drift()` を追加。`src/cli.rs::DoctorArgs` に `--fix`（remediation plan を計算してプレビュー表示）と `--apply`（`requires = "fix"`、低リスク・自動適用可能な修復のみ実行。今回は stale PyPI cache の `pybun gc` 相当の削除のみ対応）を追加。`src/commands/maintenance.rs::run_doctor()` で Python 未検出・stale PyPI cache 検出時にそれぞれ対応する `FixCandidate` 付き `Diagnostic` を生成し、`--fix` 時は `detail.fix_plan` に、`--apply` 時は実際に適用した結果を `detail.applied_fixes` に出力。
  - Tests: TDD で `src/schema.rs`（`test_diagnostic_with_fix_candidates` 等3件）、`src/self_heal.rs`（`missing_python_fix_is_not_auto_applicable` 等3件）の unit test を追加し全件 green。新規 `tests/doctor_fix.rs` に5件の統合テスト（`--help` 表示、`--apply` 単独指定の拒否、healthy 環境での空 plan、stale cache 検出時の `fix_plan`/`fix_candidates` 内容、`--apply` による実削除）を追加し、`PYBUN_PYPI_CACHE_DIR` で実キャッシュから隔離。`cargo fmt -- --check` / `cargo clippy --all-targets --all-features -- -D warnings` / `cargo test`（フルスイート、`tests/compat_snapshots.rs` の `help_doctor.txt` / `json_schema_print.json` スナップショットを新しい `--fix`/`--apply` ヘルプ文言と `fix_candidates` schema 定義に追従して更新）すべてパス。
  - Remaining: `install`/`run`/`build`/`test` への `--explain`/`--plan` 展開、MCP ツールでの remediation plan 露出、lock drift の自動適用（現在は Medium risk のため手動確認が必要）は follow-up PR で対応する。

- [DONE] PR-DRIFT1: Dependency drift detection via AST import analysis (Issue #248)
  - Goal: エージェントが生成したコードの「宣言漏れ import」と「未使用の依存宣言」を静的解析で検出し、`pybun drift` CLI と `pybun_drift` MCP ツール、`pybun_context` の `drift_summary` として公開する。
  - Current: `src/drift.rs` を新設。`analyze(root)` が正規表現/トークンベースの Phase 1 実装で Python ファイルを再帰スキャンし、stdlib 除外・インポートエイリアスマッピング（PIL→Pillow, cv2→opencv-python 等20種）・パッケージ名正規化を行う。`UndeclaredImport`（`next_action: {tool: "pybun_add", ...}`）と `UnusedDeclaration`（`next_action: {tool: "pybun_remove", ...}`）を返す。`src/cli.rs` に `Commands::Drift(DriftArgs)` を追加。`src/commands/mod.rs::run_drift()` が `E_DRIFT_NO_PYPROJECT` 診断付きエラー、undeclared/unused の `W_DRIFT_*` 警告、`detail.{undeclared_imports, unused_declarations, analysis_notes, files_scanned}` を含む JSON を返す。`src/mcp.rs` に `pybun_drift` ツールと `call_drift()` を追加し、`call_context()` の `drift_summary.undeclared_imports: null` を `include_drift: true` 時に実際の分析結果へ置換。`cwd` パラメータで作業ディレクトリを指定可能。
  - Tests: `src/drift.rs` 内ユニットテスト24件（parse_import_line各パターン、stdlib判定、エイリアスマッピング、パッケージ名正規化、analyze undeclared/unused/clean/stdlib除外/PIL解決/next_action検証、collect_py_files）。`tests/drift.rs` 統合テスト16件 — CLI: undeclared/unused検出、クリーンプロジェクト、stdlib除外、PILエイリアス、from構文、location情報、next_action、pyproject不在エラー、テキスト出力、サブディレクトリ、analysis_notes。MCP: tools/listにpybun_driftが含まれること、構造化結果の確認、pybun_contextのinclude_drift=true時のdrift_summary。全テスト・`cargo clippy --all-targets --all-features -- -D warnings`・`cargo fmt -- --check` がパス。
...
- [DONE] PR1.2: Resolver core (SAT solver and index client abstraction).
  - Current: Full version specifier support (==, >=, >, <=, <, !=, ~=) is implemented in `src/specifier.rs` on top of the PEP 440 version type in `src/pep440.rs`; `src/resolver.rs` retains compatibility re-exports. JSON fixture loading is available through an explicit CLI `--index` path, and network-backed PyPI metadata caching is owned separately by `PyPiClient`. The orphaned `IndexCache` / `CachedIndexLoader` subsystem was removed in Issue #408. In-memory indexes use the highest-version selection strategy.
  - Depends on: PR1.1.  
