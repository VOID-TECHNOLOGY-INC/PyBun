...
- [IN PROGRESS] PR-A10: `src/commands.rs` を per-command モジュールへ分割 (Issue #186)
  ...
  - Current（Issue #271 follow-up）: `pybun add` / `pybun remove` / `pybun install` / `pybun outdated` / `pybun upgrade` の実装を `src/commands/install.rs` へ抽出し、`mod install;` を宣言。`mod.rs` 側の dispatcher を `install::run_add(...)` / `install::run_remove(...)` / `install::run_install(...)` / `install::run_outdated(...)` / `install::run_upgrade(...)` 呼び出しへ更新。新モジュールから直接参照される構造体・フィールドは `pub(crate)` 化（`super::` 経由の private アクセスでは不足したもののみ）。JSON detail / diagnostics / summary は変更なし。`mod.rs` は 4903行 → 3798行に縮小、新設の `install.rs` は 1662行。
